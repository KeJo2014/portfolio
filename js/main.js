...
app.page = {
    ...
    initPage: function () {
        ...
        app.conatct_form = require('./app/contact-form.js');
    ...
    loadPage: function (rel, id) {
        ...
        id = "section-about"
        if (app.ajax !== null) {
            app.preloader.preloaderInit("about");
            return false;
...
app.preloader = {
    ...
    preloaderInit: function (rel) {
        ...
        app.preloader.animations.preloaderanimHide = new TimelineMax().fromTo($ajaxify, 0.5, { immediateRender: false, opacity: 0.3 }, { opacity: 1 }, 0.3).fromTo($preloader, 0.6, { immediateRender: false, x: '0%', ease: Power4.easeIn }, {
...
var isInViewport = function (elem) {
    ...
    return (
        bounding.left >= 0 &&
        bounding.bottom <= (window.innerHeight+400 || document.documentElement.clientHeight+1000) &&
...
sections = [
    ["section-home", document.querySelector("#section-home")],
    ["section-qualification", document.querySelector("#section-qualification")],
    ["section-about", document.querySelector("#section-about")],
    ["section-timeline", document.querySelector("#section-timeline")],
    ["section-contact", document.querySelector("#section-contact")]
]

onscroll = function () {
    for (let index = 0; index < sections.length; index++) {
        if (isInViewport(sections[index][1])) {
            // if changed start anim
            if(sections[index][0] != "section-"+currentPage){
                window.history.pushState("","","?page="+sections[index][0])
                switch (sections[index][0]) {
                    case "section-qualification":
                        currentPage = "qualification"
                        nav_item = document.querySelector("#nav-qualifications")
                        break;
                    case "section-about":
                        currentPage = "about"
                        nav_item = document.querySelector("#nav-about")
                        break;
                    ...
                    case "section-contact":
                        currentPage = "contact"
                        nav_item = document.querySelector("#nav-contact")
                        break;
                }
            }
            //set all nav items to unselected
            document.querySelector("#nav-qualifications").style = "color: #909096;";
            document.querySelector("#nav-about").style = "color: #909096;";
            document.querySelector("#nav-timeline").style = "color: #909096;";
            document.querySelector("#nav-contact").style = "color: #909096;";
            //select current nav
            var nav_item = null;
            switch (sections[index][0]) {
                case "section-qualification":
                ...
                case "section-timeline":
                    currentPage = "time"
                    nav_item = document.querySelector("#nav-timeline")
                    break;
                case "section-contact":
                    currentPage = "contact"
                    nav_item = document.querySelector("#nav-contact")
